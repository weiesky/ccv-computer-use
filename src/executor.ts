...
export interface ComputerExecutorCapabilities {
  ...
  hostBundleId: string
  /**
   * Expose teach-mode tools (`request_teach_access`, `teach_step`, `teach_batch`)
   * in `buildComputerUseTools`. Standalone host enables this with no GUI
   * overlay — the session context auto-advances each step instead of waiting
   * for a Next click.
   */
  teachMode?: boolean
  /**
   * macOS-only: true when Xcode Command Line Tools are installed AND at
   * least one iOS Simulator is registered. Triggers inclusion of the
   * `ios_*` tool set in `buildComputerUseTools`.
   */
