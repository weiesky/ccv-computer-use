...
export interface ComputerUseHostAdapter {
   ...
   * Injected so the pixel-compare seam stays host-side. The desktop host
   * implements it via Electron's `nativeImage.createFromBuffer(jpeg).crop(rect).toBitmap()`;
   * this standalone package implements it via `sharp` (libvips) — async
   * because sharp's pipeline is Promise-based.
   *
