...
import { dispatchIOSTool, isIOSTool } from './iosToolHandlers.js'
...
async function dispatchAction(
  ...
  // ── iOS Simulator tools (early intercept) ──────────────────────────
  // iOS tools drive the Simulator.app guest OS, not macOS apps. They bypass
  // the frontmost-app gate, allowlist, hide-before-action, and pixel-compare
  // gates — those are all designed for "is the action targeting an app the
  // user approved on this Mac", which doesn't apply to a virtualized iPhone.
  // TCC + kill-switch + CU-lock gates already ran in handleToolCall before
  // we got here.
  if (isIOSTool(name)) {
    const result = await dispatchIOSTool(name, a)
    return result as CuCallToolResult
  }
