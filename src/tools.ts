...
export function buildComputerUseTools(
  caps: {
    ...
    teachMode?: boolean
    /** Append ios_* tools (macOS-only). Caller detects Xcode/simulators once at startup. */
  ...
  return [
    ...
    ...(caps.iosSimulatorAvailable ? buildIOSTools() : []),
  ]
}
...
/**
 * iOS Simulator tools. Exposed only when `caps.iosSimulatorAvailable` is true
 * (host is macOS, Xcode CLT installed, at least one simulator registered).
 * These tools are INDEPENDENT of the macOS desktop tools above — they drive
 * the Simulator.app process via `xcrun simctl` and (optionally) Facebook's
 * `idb`. None of them consult the session allowlist: the simulator is a
 * sandboxed guest OS, not the host Mac.
 *
 * Coordinate convention: device logical points. The simulator screenshot
 * returns native-pixel dimensions; the host divides by `scaleFactor` from
 * `ios_get_device_info` to convert. We do NOT support `normalized_0_100`
 * for iOS today — models should call `ios_screenshot` first and read
 * coordinates directly off the image.
 */
function buildIOSTools(): Tool[] {
  const udidParam = {
    type: 'string',
    description:
      'Target simulator UDID. Omit to use the currently booted simulator (error if none booted).',
  }
  ...
  return [
    // ── Simulator management ───────────────────────────────────────────
    {
      ...
      description:
        'List all registered iOS Simulators with their UDID, name, state (Booted/Shutdown), device type, and iOS runtime version. Use this to find a UDID for other ios_* tools.',
      inputSchema: {
        type: 'object' as const,
        properties: {},
        required: [],
    ...
    },
    {
      ...
      description:
        'Boot an iOS Simulator and open Simulator.app. Idempotent — calling on an already-booted device succeeds silently. Waits up to 10s for the Booted state.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: {
            ...udidParam,
            description:
              'UDID of the simulator to boot. Omit to boot the first available iPhone.',
          },
        },
        required: [],
      },
    },
    {
      ...
      description:
        'Shut down a booted iOS Simulator. Safe to call on an already-shutdown device.',
      inputSchema: {
        type: 'object' as const,
        properties: { udid: { ...udidParam, description: 'UDID of the simulator to shut down (required).' } },
        required: ['udid'],
      },
    },
    {
      ...
      description:
        'Get the currently booted simulator\'s info: UDID, name, device type (e.g. "iPhone 15 Pro"), iOS runtime version, screen size, scale factor. Useful for converting screenshot pixels to logical points.',
      inputSchema: {
        type: 'object' as const,
        properties: { udid: udidParam },
        required: [],
      },
    },
    ...
    // ── Screen & UI ───────────────────────────────────────────────────
    {
      name: 'ios_screenshot',
      description:
        'Take a screenshot of the iOS Simulator screen. Returns base64-encoded JPEG (preferred) or PNG with pixel dimensions. Coordinates for ios_tap / ios_swipe are read off this image directly.',
      inputSchema: {
        type: 'object' as const,
        properties: { udid: udidParam },
        required: [],
    ...
    },
    {
      ...
      description:
        'Get the accessibility element tree of the foreground app on the simulator (types, labels, identifiers, frames). Useful for finding elements by name instead of guessing coordinates. REQUIRES Facebook idb — install with: brew install idb-companion && pipx install fb-idb.',
      inputSchema: {
        type: 'object' as const,
        properties: { udid: udidParam },
        required: [],
    ...
    // ── Touch interactions ────────────────────────────────────────────
    {
      name: 'ios_tap',
      description:
        'Tap the simulator screen at logical device coordinates (x, y). REQUIRES Facebook idb.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
          x: {
            type: 'number',
            description: 'Horizontal position in logical points (from screenshot / device scale).',
          },
          y: {
            type: 'number',
            description: 'Vertical position in logical points.',
          },
        },
        required: ['x', 'y'],
      },
    },
    {
      name: 'ios_long_press',
      description:
        'Long-press the simulator screen at logical device coordinates for a duration. REQUIRES Facebook idb.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
          x: { type: 'number', description: 'Horizontal position in logical points.' },
          y: { type: 'number', description: 'Vertical position in logical points.' },
          duration_ms: {
            type: 'number',
    ...
    },
    {
      name: 'ios_swipe',
      description:
        'Swipe from (x1, y1) to (x2, y2) on the simulator. Use for scrolling lists, paging carousels, pull-to-refresh. REQUIRES Facebook idb.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
          x1: { type: 'number', description: 'Start X in logical points.' },
          y1: { type: 'number', description: 'Start Y in logical points.' },
          x2: { type: 'number', description: 'End X in logical points.' },
          y2: { type: 'number', description: 'End Y in logical points.' },
          duration_ms: {
            type: 'number',
            description: 'Swipe duration in milliseconds. Lower = faster flick. Default 250.',
          },
        },
    ...
    },
    {
      name: 'ios_type_text',
      description:
        'Type text into the focused text field on the simulator. Tap a field first to focus it. REQUIRES Facebook idb.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
    ...
    },
    {
      ...
      description:
        'Press a hardware button on the simulator (home, lock, siri, volume-up, volume-down). REQUIRES Facebook idb.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
          button: {
            type: 'string',
            enum: ['home', 'lock', 'siri', 'volume-up', 'volume-down'],
            description: 'Which hardware button to press.',
          },
        },
        required: ['button'],
      },
    ...
    // ── App management ────────────────────────────────────────────────
    {
      name: 'ios_launch_app',
      description:
        'Launch an app on the simulator by bundle id (e.g. "com.apple.mobilesafari"). Returns the launched PID.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
          bundle_id: {
            type: 'string',
            description: 'Bundle identifier of the installed app.',
          },
        },
        required: ['bundle_id'],
      },
    },
    {
      ...
      description:
        'Terminate a running app on the simulator by bundle id. Errors if the app is not running.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
    ...
    },
    {
      ...
      description:
        'Install a .app bundle on the simulator. Path is on the HOST filesystem (must be a simulator-arch build, not an App Store / device .ipa).',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
          app_path: {
            type: 'string',
            description: 'Absolute path to a .app bundle on the host.',
          },
        },
        required: ['app_path'],
      },
    ...
    // ── Location ──────────────────────────────────────────────────────
    {
      ...
      description:
        'Set the simulator\'s GPS location. Persists until ios_clear_location. Apps using CoreLocation will see this position.',
      inputSchema: {
        type: 'object' as const,
        properties: {
          udid: udidParam,
    ...
    {
      name: 'ios_clear_location',
      description:
        'Clear a previously-set simulator location. The device returns to "no location" / Cupertino default.',
      inputSchema: {
        type: 'object' as const,
        properties: { udid: udidParam },
        required: [],
