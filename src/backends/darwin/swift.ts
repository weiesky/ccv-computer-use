...
function jxaSync(script: string): string {
  return runSync(['osascript', '-l', 'JavaScript', '-e', script])
}

function osascriptSync(script: string): string {
  return runSync(['osascript', '-e', script])
}
...
async function jxa(script: string): Promise<string> {
  return runAsync(['osascript', '-l', 'JavaScript', '-e', script])
}
...
async function captureScreenToBase64(
  ...
  await runAsync(['screencapture', ...args, tmpFile])
  try {
