 ...
 * The original used Electron's `nativeImage`; this standalone package uses
 * `sharp` (libvips) — the crop is still injected so the seam stays host-side
 * and tests can substitute a fake decoder. Async because sharp's pipeline
 * is Promise-based.
 */
